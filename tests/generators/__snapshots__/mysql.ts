...
import { bigint, boolean, datetime, decimal, double, foreignKey, int, json, mysqlEnum, mysqlTable, primaryKey, uniqueIndex, varchar } from 'drizzle-orm/mysql-core'

export const User = mysqlTable('users', {
	id: int('id').notNull().primaryKey().autoincrement(),
	email: varchar('email', { length: 191 }).notNull().unique(),
	name: varchar('full_name', { length: 191 }),
	age: int('age').notNull().default(18),
	balance: double('balance').notNull().default(0.5),
	...
	uuid: varchar('uuid', { length: 191 }).notNull().default(sql`uuid()`),
	cuid: varchar('cuid', { length: 191 }).notNull().default(sql`cuid()`),
	generated: varchar('generated', { length: 191 }).notNull().default(sql`gen_random_uuid()`),
...
export const Post = mysqlTable('posts', {
	id: int('id').notNull().primaryKey(),
	title: varchar('post_title', { length: 191 }).notNull(),
...
export const TagToUser = mysqlTable('_TagToUser', {
